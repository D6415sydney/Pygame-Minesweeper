...
def uncover_all_adj():
    for row in range(rows):
        for col in range(cols):
            ...
            rowc = row - 1
            colc = col - 1
            if game_map[rowc][colc] == 1 and [rowc, colc] not in mines and find_mines(rowc, colc) == 0:
                #print()
                if rowc != 0:
                    if colc != 0:
                        game_map[rowc - 1][colc - 1] = 1
                    if colc != cols:
                        game_map[row - 1][colc + 1] = 1
                    game_map[rowc - 1][colc] = 1
                if rowc != rows:
                    if colc != 0:
                        game_map[rowc + 1][colc - 1] = 1
                    if colc != cols - 1:
                    ...
                    game_map[rowc + 1][colc] = 1
                if colc != 0:
                    game_map[rowc][colc - 1] = 1
                if colc != cols - 1:
                    game_map[rowc][colc + 1] = 1
                game_map[rowc][colc] = 1

    
def uncover_all_adjo():
    global game_map
    rown = 0
    coln = 0
    for rown in range(rows):
        for coln in range(cols):
            row = rown - 1
            col = coln - 1
            if game_map[row][col] == 1 and find_mines(row, col) == 0:
                #print(col, row)
                if row != 0:
                    if col != 0:
