...
# Open source and copyright free (after November 7th, 23:59)
...
def show_text(content,size,color,position):
    ...
    elif "lucidasanstypewriter" in pygame.font.get_fonts():
        font=pygame.font.SysFont("lucidasanstypewriter",int(2*size))
        text_surface=font.render(content,True,color)
        screen.blit(text_surface,position)
    else:
        font=pygame.font.SysFont("freesansbold",int(2.5*size))
        text_surface=font.render(content,True,color)
        screen.blit(text_surface,position)
...
def main(r,c,m):
    ...
    while True:
        ...
        for ev in pygame.event.get():
            ...
            # Detect click on smiley
            if ev.type == pygame.MOUSEBUTTONDOWN and pygame.mouse.get_pressed()[0] and mouse[0] >= screen_width/2 - 30 and mouse[0] <= screen_width/2 + 30 and mouse[1] >= (ybuf+panh/2)*tile_size+br-30 and mouse[1] <= (ybuf+panh/2)*tile_size+br + 30:
                    init_game(cols, rows)
...
l_version = tk.Label(root, text="VERSION 9",font=("Press Start 2P", 10),fg='red', bg = 'black')
l_version.place(x=10,y=10)
